...
for (var index = 0; index < questions.length; index++) {
    if(choices[index].id !== "first") {
        choices[index].innerHTML += '<a href="#" style="margin:115px -453px" class="leftArrow" onclick="onRewind();">back</a>';
    }
}
    ...
    for (var i = 0; i < otherBtns.length; i++) {
        ...
        if (clicked  !== otherBtns){
            ...
            if (clicked.style.backgroundColor === "rgb(255, 255, 255)") {
                var nextQuestion =  document.getElementsByClassName("choices");
                nextQuestion[n].classList.remove("visible");
            }
        }
    }
}
